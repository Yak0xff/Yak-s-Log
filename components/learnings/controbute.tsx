...
const GitHubControbute = () => {
  ...
  const monthList: Array<HTMLLIElement> = []
  months.forEach((month, index) => {
    monthList.push(<li key={index}>{month}</li>)
  })

  const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
  const dayList: Array<HTMLLIElement> = []
  days.forEach((day, index) => {
    dayList.push(<li key={index}>{day}</li>)
  })

  const levelList: Array<HTMLLIElement> = []
  for (let i = 1; i < 365; i++) {
    ...
    <>
      <div className="pt-6 text-center xl:pb-4">
        ...
        <div className="graph">
          ...
          <ul className="days">{dayList}</ul>
          <ul className="squares">{levelList}</ul>
        </div>
      </div>
    </>
  )
